...
RF.applySavedSettingsToUI = function () {
    var s = RF.state.settings;
    var validFonts = Array.from(document.querySelectorAll('.font-option')).map(function (el) {
        return el.dataset.font;
    });
    if (validFonts.indexOf(s.font) === -1 && validFonts.length) {
        s.font = RF.DEFAULT_SETTINGS.font;
    }
